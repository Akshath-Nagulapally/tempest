type Features = {
  ...
  'Easy Scalability': boolean
  'Hosting Servers': boolean
}

const data: { service: string; features: Features }[] = [
  {
    service: 'Us⭐',
    features: {
      'User Interface': true,
          ...
          <tr key={feature}>
            ...
            {data.map((serviceData) => (
              <td key={serviceData.service} className="px-4 py-2">
                {serviceData.features[feature as keyof Features] ? (
