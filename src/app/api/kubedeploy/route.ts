...
export function GET(request: Request) {
          ...
          containers: [
            {
              ...
              image:
                'akshathnag06002/akkiregistry:goarklandingpage_nix_builder_test', //CHANGE THIS TO WHATEVER IS REQUIRED BOSS
            },
          ],
          imagePullSecrets: [
            {
  ...
  const ingress = {
    ...
    spec: {
      ...
      tls: [
        {
          hosts: [`${FunctionalProjectName}.akkiai.com`],
  ...
  if (FunctionalProjectName.length > 63) {
    return new Response('Project name must be 63 characters or less', {
      status: 400,
    })
  }

  async function createOrPatch(resource: any) {
    try {
  ...
  return new Response(
    `Successfully created deployment for: ${FunctionalProjectName}`,
  )
}
